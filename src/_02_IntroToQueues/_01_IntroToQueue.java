...
public class _01_IntroToQueue {
	public static void main(String[] args) {
		...
		for (int i = 0; i < 100; i++) {
			doubleStack.push((Math.random() * 100));
		}
		...
		doubleQueue.add(doubleStack.pop());
		...
		while (!doubleStack.empty() || !doubleQueue.isEmpty()) {
			if (!doubleStack.empty()) {
				System.out.println(doubleStack.pop());
			}
			if (!doubleQueue.isEmpty()) {
				System.out.println(doubleQueue.pop());
			}
		}
